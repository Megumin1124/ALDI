...
class AlignMixin(GeneralizedRCNN):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #图像级特征在卷积层之后，实例级特征在全连接层之后,ALDI++实际上不会执行Align操作
        ...
        if self.training:
            if do_align:
                ...
                if self.img_align:
                    ...
                    #loss = SymmetricUnifiedFocalLoss(domain_preds, torch.FloatTensor(domain_preds.data.size()).fill_(domain_label).to(device))
