...
class DatasetMapper(_DatasetMapper):
    def __call__(self, dataset_dict):
        ...
        # 如果sem_seg_file_name在字典属性中，则加载为单通道灰度图
        ...
        if "sem_seg_file_name" in dataset_dict:
            ...
            sem_seg_gt = None
        """数据增强与变换：
        构造 AugInput 对象，将图像和语义分割标签包装起来。
        应用增强self.augmentations,对 image 和 sem_seg 进行变换。"""    
    
        aug_input = T.AugInput(image, sem_seg=sem_seg_gt)
        transforms = self.augmentations(aug_input)
        image, sem_seg_gt = aug_input.image, aug_input.sem_seg
        """数据转换与存储：
        将增强后的图像转换为 PyTorch 张量，并将通道维度置于最前。
        如果语义分割标签存在，也进行转换并保存。
        """
        ...
        """
        提案处理：
        如果启用了 proposal_topk 参数，应用变换到建议框上。
        """
        ...
        """
        推理模式下,移除标注信息annotations 和 sem_seg_file_name,直接返回数据字典。
        """
        if not self.is_train:
            ...
            return dataset_dict
        """
        训练模式对目标标注框bounding boxes应用与图像一致的变换
        """
