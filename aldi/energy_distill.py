...
def build_distiller(cfg, teacher, student):
    # cfg.DOMAIN_ADAPT.DISTILL.DISTILLER_NAME = ALDIDistiller
    # 返回 ALDIDistiller 类的 from_config 的返回值
...
class ALDIDistiller(Distiller):
    ...
    def __init__(self, teacher, student, do_hard_cls=False, do_hard_obj=False, do_hard_rpn_reg=False, do_hard_roi_reg=False,
        ...
        self.register_hooks()
        self.pseudo_labeler = PseudoLabeler(teacher, pseudo_label_threshold) #返回伪标签

    @classmethod
    def from_config(cls, cfg, teacher, student):
        return ALDIDistiller(teacher, student,
                        do_hard_cls=cfg.DOMAIN_ADAPT.DISTILL.HARD_ROIH_CLS_ENABLED,#false
                        do_hard_obj=cfg.DOMAIN_ADAPT.DISTILL.HARD_OBJ_ENABLED,#false
                        do_hard_rpn_reg=cfg.DOMAIN_ADAPT.DISTILL.HARD_RPN_REG_ENABLED,#false
                        do_hard_roi_reg=cfg.DOMAIN_ADAPT.DISTILL.HARD_ROIH_REG_ENABLED,#false
                        do_cls_dst=cfg.DOMAIN_ADAPT.DISTILL.ROIH_CLS_ENABLED, #true
                        do_obj_dst=cfg.DOMAIN_ADAPT.DISTILL.OBJ_ENABLED,#true
                        do_rpn_reg_dst=cfg.DOMAIN_ADAPT.DISTILL.RPN_REG_ENABLED,#true
                        do_roih_reg_dst=cfg.DOMAIN_ADAPT.DISTILL.ROIH_REG_ENABLED,#true
    ...
    def get_roih_losses(self):
        ...
        """
        student_cls_logits tensor([[-0.9980,  0.9990],
        [-1.4629,  1.5312],
        ...,
        [-1.1143,  1.2051]], device='cuda:1', dtype=torch.float16,grad_fn=<AddmmBackward0>) 
       student_proposal_deltas tensor([[-0.2617,  0.0119,  0.1164,  0.0723],   边界框偏移量dx,dy,dh,,dw
        [-0.2505,  0.2944,  0.2189,  0.5693],
        ...,
        [-0.4673,  1.0811, -0.6714,  0.9170]], device='cuda:1',dtype=torch.float16, grad_fn=<AddmmBackward0>)
        """
        energy_teacher = teacher_cls_logits.logsumexp(1)
        energy_real_teacher = energy_teacher.mean()

        energy_student = student_cls_logits.logsumexp(1)
        exergy_real_student = energy_student.mean()

        """print("energy_real_teacher:",energy_real_teacher)
        print("exergy_real_student",exergy_real_student)"""
        loss = abs(energy_real_teacher - exergy_real_student)
        losses["loss_roih_energy"] = loss/10

        
    
        # 接下来计算energy_fake
        # 1.计算inputs_fake，然而inputs_fake实际上是模拟源数据，所以应当可以拿源数据直接做x_fake

        
        # 2.计算logits_fake,energy_fake

        """energy: tensor([1.6001, 1.7845, 1.6163,  ..., 2.9369, 1.0424, 1.1196], device='cuda:0')"""
...
"""
teacher_batched_inputs : List[Dict[str, torch.Tensor]]
[{'file_name': 'datasets/cityscapes/leftImg8bit/train/krefeld/krefeld_000000_034231_leftImg8bit.png', 
'height': 1024, 'width': 2048, 
'image_id': 'krefeld_000000_034231_leftImg8bit.png', 
'image': tensor([[[ 61, 62, 63, ..., 0, 0, 0], [ 63, 64, 66, ..., 0, 0, 0], [ 58, 59, 61, ..., 0, 0, 0], ..., [ 36, 35, 37, ..., 38, 38, 38], [ 35, 35, 37, ..., 41, 38, 37], [ 35, 36, 37, ..., 44, 43, 42]], [[ 79, 79, 80, ..., 0, 0, 0], [ 85, 85, 85, ..., 0, 0, 0], [ 72, 73, 75, ..., 0, 0, 0], ..., [ 40, 40, 40, ..., 42, 43, 43], [ 40, 40, 40, ..., 46, 43, 41], [ 40, 40, 40, ..., 48, 47, 46]], [[125, 125, 124, ..., 0, 0, 0], [125, 125, 124, ..., 0, 0, 0], [118, 119, 120, ..., 0, 0, 0], ..., [ 31, 31, 31, ..., 32, 33, 33], [ 31, 31, 31, ..., 36, 34, 31], [ 31, 31, 31, ..., 38, 37, 36]]], dtype=torch.uint8), 
'instances': Instances(num_instances=0, image_height=832, image_width=1664, fields=[gt_boxes: Boxes(tensor([], size=(0, 4))), gt_classes: tensor([], dtype=torch.int64)]), 
'img_weak': tensor([[[ 61, 62, 63, ..., 0, 0, 0], [ 63, 64, 66, ..., 0, 0, 0], [ 58, 59, 61, ..., 0, 0, 0], ..., [ 36, 35, 37, ..., 38, 38, 38], [ 35, 35, 37, ..., 41, 38, 37], [ 35, 36, 37, ..., 44, 43, 42]], [[ 79, 79, 80, ..., 0, 0, 0], [ 85, 85, 85, ..., 0, 0, 0], [ 72, 73, 75, ..., 0, 0, 0], ..., [ 40, 40, 40, ..., 42, 43, 43], [ 40, 40, 40, ..., 46, 43, 41], [ 40, 40, 40, ..., 48, 47, 46]], [[125, 125, 124, ..., 0, 0, 0], [125, 125, 124, ..., 0, 0, 0], [118, 119, 120, ..., 0, 0, 0], ..., [ 31, 31, 31, ..., 32, 33, 33], [ 31, 31, 31, ..., 36, 34, 31], [ 31, 31, 31, ..., 38, 37, 36]]], dtype=torch.uint8)},
{'file_name': 'datasets/cityscapes/leftImg8bit/train/krefeld/krefeld_000000_034231_leftImg8bit.png', 
'height': 1024, 'width': 2048, 
'image_id': 'krefeld_000000_034231_leftImg8bit.png', 
'image': tensor([[[ 61, 62, 63, ..., 0, 0, 0], [ 63, 64, 66, ..., 0, 0, 0], [ 58, 59, 61, ..., 0, 0, 0], ..., [ 36, 35, 37, ..., 38, 38, 38], [ 35, 35, 37, ..., 41, 38, 37], [ 35, 36, 37, ..., 44, 43, 42]], [[ 79, 79, 80, ..., 0, 0, 0], [ 85, 85, 85, ..., 0, 0, 0], [ 72, 73, 75, ..., 0, 0, 0], ..., [ 40, 40, 40, ..., 42, 43, 43], [ 40, 40, 40, ..., 46, 43, 41], [ 40, 40, 40, ..., 48, 47, 46]], [[125, 125, 124, ..., 0, 0, 0], [125, 125, 124, ..., 0, 0, 0], [118, 119, 120, ..., 0, 0, 0], ..., [ 31, 31, 31, ..., 32, 33, 33], [ 31, 31, 31, ..., 36, 34, 31], [ 31, 31, 31, ..., 38, 37, 36]]], dtype=torch.uint8), 
...
"""
    ...
    print("loss_distill:",losses)
    {'loss_obj_bce': tensor(0.0543, device='cuda:0',grad_fn=<BinaryCrossEntropyWithLogitsBackward0>), 
    'loss_rpn_l1': tensor(0.0876, device='cuda:0', dtype=torch.float16, grad_fn=<MeanBackward0>),
    'loss_cls_ce': tensor(0.3532, device='cuda:0', grad_fn=<DivBackward1>),
    'loss_roih_l1': tensor(0.2189, device='cuda:0', grad_fn=<DivBackward0>)}
"""

"""
    print("energy_loss:",loss)
    energy_real_teacher: tensor(2.4446, device='cuda:0')
    exergy_real_student tensor(1.9917, device='cuda:0', grad_fn=<MeanBackward0>)
    energy_loss: tensor(-0.4529, device='cuda:0', grad_fn=<NegBackward0>)
"""