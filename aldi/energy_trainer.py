...
from aldi.energy_distill import build_distiller
...
# 如果没有提供无标签数据（unlabeled_weak 和 unlabeled_strong），则进行标准的监督学习。
# 如果提供了无标签数据，则通过教师模型生成伪标签，并进行训练
def run_model_labeled_unlabeled(trainer, labeled_weak, labeled_strong, unlabeled_weak, unlabeled_strong):
     ...
     """
     do_weak: 使用弱增强的有标签数据进行训练。
     do_strong: 使用强增强的有标签数据进行训练。
     do_align: 如果模型需要进行对齐（如图像对齐），则执行图像对齐。
     do_distill: 如果启用了蒸馏(teacher-student模型)，则使用教师模型生成伪标签并进行蒸馏训练
     """
     _model = model.module if type(model) == DDP else model
     # print(type(model)) <class 'torch.nn.parallel.distributed.DistributedDataParallel'>
     #print(_model)
     #assert 1>2
     do_weak = labeled_weak is not None # False
     do_strong = labeled_strong is not None # True
  
     #检查_model中是否有"img_align", "ins_align"，只要有其中一个，就返回True
     do_align = any( [ getattr(_model, a, None) is not None for a in ["img_align", "ins_align"] ] ) # false
     do_distill = trainer.distiller.distill_enabled() #true
     ...
     def add_to_loss_dict(losses, suffix, key_conditional=lambda k: True):
          ...
          """
          train_losses: {
          'loss_cls': tensor(0.0989, device='cuda:0', grad_fn=<MulBackward0>), 
          'loss_box_reg': tensor(0.1407, device='cuda:0', grad_fn=<MulBackward0>), 
          'loss_rpn_cls': tensor(0.0995, device='cuda:0', grad_fn=<MulBackward0>), 
          'loss_rpn_loc': tensor(0.0615, device='cuda:0', grad_fn=<MulBackward0>)}
          distill_loss: 
          {'loss_cls': tensor(0., device='cuda:0', grad_fn=<MulBackward0>), 
          'loss_box_reg': tensor(0., device='cuda:0', grad_fn=<MulBackward0>), 
          'loss_rpn_cls': tensor(0., device='cuda:0', grad_fn=<MulBackward0>), 
          'loss_rpn_loc': tensor(0., device='cuda:0', grad_fn=<MulBackward0>), 
          'loss_obj_bce': tensor(0.0818, device='cuda:0',grad_fn=<BinaryCrossEntropyWithLogitsBackward0>), 
          'loss_rpn_l1': tensor(0.0992, device='cuda:0', dtype=torch.float16, grad_fn=<MeanBackward0>), 
          'loss_cls_ce': tensor(0.4743, device='cuda:0', grad_fn=<DivBackward1>), 
          'loss_roih_l1': tensor(0.2375, device='cuda:0', grad_fn=<DivBackward0>)}
          """
     ...
     def do_distill_step(teacher_data, student_data, name="", key_conditional=lambda k: True, **kwargs):
          ...
          for batch_i in range(0, len(teacher_data), model_batch_size):
               ...
               '''
               with open('/workspace/aldi/distill_loss.txt', 'w') as f:
                    f.write(f"Stu.shape{distill_loss}\n")
               
                    f.write(f"distill_loss\n")
     ...
     if do_strong:
          ...
          #do_training_step(labeled_strong, "source_strong", True, False)
     ...
     #print(loss_dict)
     """
     {'loss_cls_source_strong': tensor(0.0855, device='cuda:0'), 
     'loss_box_reg_source_strong': tensor(0.1390, device='cuda:0'),
     'loss_rpn_cls_source_strong': tensor(0.0438, device='cuda:0'), 
     'loss_rpn_loc_source_strong': tensor(0.0647, device='cuda:0'), 
     'loss_cls_distill': tensor(0., device='cuda:0'), 
     'loss_box_reg_distill': tensor(0., device='cuda:0'), 
     'loss_rpn_cls_distill': tensor(0., device='cuda:0'), 
     'loss_rpn_loc_distill': tensor(0., device='cuda:0'), 
     'loss_obj_bce_distill': tensor(0.0334, device='cuda:0'), 
     'loss_rpn_l1_distill': tensor(0.0608, device='cuda:0', dtype=torch.float16), 
     'loss_cls_ce_distill': tensor(0.1362, device='cuda:0'), 
     'loss_roih_l1_distill': tensor(0.0645, device='cuda:0')}"""
     #assert 1>2
     return loss_dict
...
class ALDITrainer(DefaultTrainer):
     ...
     def build_train_loader(cls, cfg):
          ...
          labeled_loader = None
          # cfg.DATASETS.TRAIN = sim10k_cars_train
